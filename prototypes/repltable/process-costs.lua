    ...
    for i, current_part in ipairs(item.cost) do
      ...
      elseif current_part.type == 'item' or current_part.type == 'var' then
        ...
        if this_part then
          ...
          --					--Stop looking through this cost now and instead come back to it later
          --					break
        ...
        if recipe then
          ...
          for _, ingredient in ipairs(ingredients) do
            ...
            if known_costs['item-' .. ingredient_name] or unknown_cost_items['item-' .. ingredient_name] or known_costs['var-' .. ingredient_name] or unknown_cost_items['var-' .. ingredient_name] then
              ...
              log('There is a replication based on the recipe "' .. current_part.target .. '" and that recipe uses the item "' .. ingredient_name .. '" but said item cannot be replicated.  Its replication cost will be treated as being zero for calculation purposes.  If you deliberately want things to be this way, please create a replication variable with the same name as the required item so that this message does not show up at every startup.')
            end
          end
