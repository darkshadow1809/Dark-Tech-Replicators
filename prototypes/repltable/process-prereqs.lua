...
local function remove_repeats(name, prerequisite_strings)
  for i = #prerequisite_strings, 1, -1 do
    ...
    if prerequisite_strings[i] == 'repl-' .. name then
...
for _, current_replication in pairs(repl_table) do
  for _, current_item in pairs(current_replication.items) do
    ...
    if not current_item.pointer then
      current_item.pointer = data.raw['item'][current_item.name] or data.raw['tool'][current_item.name] or data.raw['repair-tool'][current_item.name] or data.raw['ammo'][current_item.name] or data.raw['armor'][current_item.name] or data.raw['capsule'][current_item.name] or data.raw['gun'][current_item.name] or data.raw['module'][current_item.name]
...
for _, current_replication in pairs(repl_table) do
  ...
  for _, current_prerequisite in pairs(prerequisite_table) do
    local skip = false
    for _, item in pairs(items_to_skip) do
      if current_prerequisite.target == item then
        skip = true
        log('"' .. item .. '" is in the skip list, skipping.')
      end
    end
    if not skip then
          ...
          if current_prerequisite.type == 'recipe' or current_prerequisite.type == 'recipe-tech-only' then
            ...
            if recipe.enabled == false or (recipe.normal and recipe.normal.enabled == false) then
              ...
              if not found_it then
                log('The recipe "' .. current_prerequisite.target .. '" is not enabled by default but is also not unlocked by any technologies.  It only requires its existing ingredients to replicate.')
              end
            else
            end
          end
...
--Check if pre-research has been researched, if so enable tech
...
if (disabled_type) then
  ...
  for _, current_replication in pairs(repl_table) do
    ...
    while i <= #prerequisite_strings do
      ...
      if string.sub(prerequisite_strings[i], 0, 5) == 'repl-' then
        --If it does, check to see if the repltech with the indicated name is disabled
        local prereq_tech = repl_table[string.sub(prerequisite_strings[i], 6)]
        if not prereq_tech.category.enabled then
          ...
          for _, prereq_of_prereq in pairs(prereq_tech.prerequisites) do
...
for repl_name, current_replication in pairs(repl_table) do
  ...
  if current_replication.category.enabled then
    ...
    for _, tech in pairs(copy_list) do
      ...
      if string.sub(tech, 0, 5) == 'repl-' then
        prereq_of_prereq = repl_table[string.sub(tech, 6)].prerequisites
      else
      ...
      if prereq_of_prereq ~= nil then
        ...
        for _, prereq in pairs(prereq_of_prereq) do
          ...
          for _, current_prerequisite in pairs(copy_list) do
            if prereq == current_prerequisite then
